...
def run(rank, n_gpus, hps):
    ...
    cs_mi_net = CLUBSample_group(192, 2, 384).cuda(rank)
    ps_mi_net = CLUBSample_group(192, 1, 384).cuda(rank)
    cp_mi_net = CLUBSample_group(2, 1, 384).cuda(rank)
    ...
    # optimizer_cs_mi_net = DDP(optimizer_cs_mi_net, device_ids=[rank])
    # optimizer_ps_mi_net = DDP(optimizer_ps_mi_net, device_ids=[rank])
    # optimizer_cp_mi_net = DDP(optimizer_cp_mi_net, device_ids=[rank])
...
def second_forward(scaler, half_type, hps, net_g, net_d, optim_g, optim_d, cs_mi_net, ps_mi_net, cp_mi_net,  c, w, f0, uv, p, spec, g, y, lengths ,volume, mel):
    ...
    with autocast(enabled=hps.train.fp16_run, dtype=half_type):
        ...
        spk_embs = gg # dim: 768
        lf0_embs = pred_lf0 # dim: 192
        xc_embs = fused# dim: 192
        z = z.detach() # dim: 192
        z_ptemp = z_ptemp # dim: 192
        z_p = z_p # dim: 192 
